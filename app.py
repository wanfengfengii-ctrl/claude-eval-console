...
def evaluation_public_description_needs_source_repair(item: Any) -> bool:
    """Use the scorer's source marker as an early attribution guard."""
    ...
    return not bool(EVALUATION_REVIEW_ATTRIBUTION_RE.search(description))
...
GENERATION_DIFFICULTY_MARGIN_GUIDANCE = """候选还必须保留明确的困难余量：固定规则表、字段校验、单次请求内遍历、简单距离或换算、排序取首项、普通 CRUD、常规接口串联和测试数量都不能单独作为主难点。按现有框架的常规写法就能完成时，应在同一工程核心内强化真实状态不变量、自定义算法判据或不可拆分跨层契约，不能靠增加第二套机制或无关模块抬高难度。"""
...
DIFFICULTY_CONTRACT_REVIEW_GUIDANCE = """同时生成 difficulty_contract，作为内部难度契约而不是题面或最终评分：axis 只能选择状态不变量、自定义算法或跨模块契约之一；hard_requirement 用一句话写明即使采用最简合规实现也不能删除的困难要求；acceptance_evidence 列出一至四个能直接验出该要求的题面场景；rejected_shortcut 写明最容易把题目降成普通 CRUD、简单字段流转或库函数拼接的捷径，以及为什么该捷径无法通过上述场景。另返回 difficulty_margin 和 hardness_basis；只有不可替代的困难机制清楚、最简合规实现仍不能把工作降为常规工程链路时，余量才能是明确困难或地狱。低于困难或困难边缘都必须令 approved=false，并交给现有定向改写或换候选流程处理。先主动设计一套最简合规实现做反证；如果这套捷径仍能通过全部验收，就必须把 estimated_task_difficulty 判为简单或中等并令 approved=false，不能签发困难契约。难度契约只用于后续核对和低难度迭代止损，不得替代交付完成后的独立难度评分。"""
...
def normalize_difficulty_contract(value: Any) -> Dict[str, Any]:
    ...
    difficulty_margin = re.sub(
        r"\s+", " ", str(value.get("difficulty_margin") or "")
    ).strip()
    if not difficulty_margin:
        # Stored contracts created before the margin gate remain readable.
        difficulty_margin = "地狱" if estimated == "地狱" else "明确困难"
    if difficulty_margin not in TASK_DIFFICULTY_MARGIN_OPTIONS:
        raise WorkflowError("难度契约缺少有效的难度余量")
    hardness_basis = re.sub(
        r"\s+", " ", str(value.get("hardness_basis") or "")
    ).strip()[:800]
    if not hardness_basis:
        hardness_basis = (difficulty_evidence or [hard_requirement])[0]
    ...
    return {
        ...
        "difficulty_margin": difficulty_margin,
    ...
    return normalize_difficulty_contract(
        {
            **raw,
            ...
            "difficulty_margin": (
                validation.get("difficulty_margin") or raw.get("difficulty_margin")
            ),
            "hardness_basis": (
                validation.get("hardness_basis") or raw.get("hardness_basis")
            ),
        }
    )
...
def repository_generation_token(repo_url: Any = "", repo_name: Any = "") -> str:
    """Return the in-process reservation token for one GitHub repository."""
    key = canonical_repository_key(repo_url, repo_name)
    return f"repo:{key}" if key else ""


def run_repository_generation_token(run_id: Any) -> str:
    """Resolve a persisted run to the repository-level generation token."""
    if not str(run_id or "").strip():
        return ""
    try:
        row = dict(run_row(str(run_id)))
    except WorkflowError:
        return ""
    return repository_generation_token(row.get("repo_url"), row.get("repo_name"))
...
def repository_prompt_history(
    ...
    """Collect every known turn prompt for the same repository across lineages."""
    target_key = canonical_repository_key(row["repo_url"], row["repo_name"])
    if not target_key:
        return []
    with db_connection() as database:
        local_rows = database.execute(
            """SELECT runs.id, runs.repo_name, runs.repo_url, runs.phase,
                      runs.container_cleaned,
                      turns.turn_number, turns.intent_type AS turn_task_type,
                 ...
                 FROM runs
                 JOIN run_turns AS turns ON turns.run_id = runs.id
                WHERE runs.deleted_at IS NULL AND turns.prompt <> ''
                ORDER BY turns.created_at DESC, runs.id DESC, turns.turn_number DESC"""
        ).fetchall()
        submitted_rows = database.execute(
            """SELECT run_id, turn_number, remote_submission_id, remote_status,
                      qc_summary
                 FROM solo_qa_submissions
    ...
    submitted_by_turn: Dict[Tuple[str, int], sqlite3.Row] = {}
    for submitted in submitted_rows:
        submitted_by_turn.setdefault(
            (str(submitted["run_id"]), int(submitted["turn_number"])),
            submitted,
        )
    local_by_prompt = {
        normalized_prompt_edge(str(local["turn_prompt"] or "")): local
        for local in local_rows
        if str(local["turn_prompt"] or "").strip()
    }
    ...
    # Add local turns first so a prompt generated by another active task is
    # visible immediately, even before the helper has submitted or synced it.
    # Submitted local turns retain their SOLO-QA reference and therefore do not
    # lose the richer remote identity when the remote-only pass runs below.
    for local in local_rows:
        local_key = canonical_repository_key(local["repo_url"], local["repo_name"])
        if not repository_keys_match(target_key, local_key):
            continue
        turn_number = int(local["turn_number"] or 0)
        submitted = submitted_by_turn.get((str(local["id"]), turn_number))
        ...
        has_product = bool(
            ...
            or (
                ...
                and str(local["turn_status"] or "") == "complete"
                and str(local["turn_prompt_id"] or "")
            )
        ...
        modules = (
            normalize_iteration_text_list(local["iteration_modules"], 4)
            if first_turn else []
        )
        reference = (
            f"SOLO-QA #{submitted['remote_submission_id']}"
            if submitted is not None
            else f"本地任务 {str(local['id'])[:8]} 第 {turn_number} 轮"
        )
        add({
            "reference": reference,
            "source": "local",
            ...
            "task_type": str(local["turn_task_type"] or ""),
            "remote_status": (
                str(submitted["remote_status"] or "") if submitted is not None else ""
            ),
            "prompt": str(local["turn_prompt"] or ""),
            "expansion_axis": (
                str(local["iteration_expansion_axis"] or "") if first_turn else ""
            ),
            "engineering_core": (
                str(local["iteration_engineering_core"] or "") if first_turn else ""
            ),
            "main_user_flow": (
                str(local["iteration_main_user_flow"] or "") if first_turn else ""
            ),
            "modules": modules,
            "qc_summary": (
    ...
    for remote in remote_rows:
        ...
        add({
            ...
            "expansion_axis": (
                ...
                and int(matching_local["turn_number"] or 0) == 1 else ""
            ),
            "engineering_core": (
...
def global_prompt_dedup_history(
    ...
    with db_connection() as database:
        ...
        local_rows = database.execute(
            """SELECT runs.id, runs.repo_name, runs.repo_url,
                      turns.prompt, turns.intent_type AS task_type,
                      runs.phase, turns.prompt_id, turns.created_at
                 FROM runs
                 JOIN run_turns AS turns ON turns.run_id = runs.id
...
def run_codex_task_generation(
    ...
    prompt += "\n" + GENERATION_DIFFICULTY_MARGIN_GUIDANCE
...
def run_codex_task_validation(
    ...
    schema = {
        ...
        "properties": {
            ...
            "hardness_basis": {"type": "string", "minLength": 8, "maxLength": 800},
            "difficulty_evidence": {
                "type": "array",
                "items": {"type": "string", "minLength": 1},
                "minItems": 1,
        ...
        "required": [
            ...
            "difficulty_margin", "hardness_basis", "difficulty_evidence",
            "difficulty_contract", "scope_review"
        ],
        "additionalProperties": False,
    }
    review_payload = {
...
def task_review_scope_errors(validation: Dict[str, Any]) -> List[str]:
    ...
    try:
        contract = reviewed_difficulty_contract(validation)
        if contract["difficulty_margin"] not in REQUIRED_TASK_DIFFICULTY_MARGINS:
            errors.append(
                "独立复核难度余量不足："
                f"{contract['difficulty_margin']}，必须为明确困难或地狱"
            )
    except WorkflowError as exc:
...
def run_codex_bugfix_validation(
    ...
    schema = {
        ...
        "required": [
            ...
            "difficulty_margin", "hardness_basis", "difficulty_evidence",
            "difficulty_contract", "bug_review"
        ],
        "additionalProperties": False,
    }
...
def run_codex_prompt_dedup_validation(
    ...
    prompt = (
        ...
        "同仓库历史按平台规则 C 判断：工程核心、主要用户操作、故障根因或最终验收结果任一项实质相同，仅更换字段、参数、页面细节或同义词，判为重复。"
        "同一主接口、同一页面流程或同一状态机继续增加可选字段、逐行校验、持久化、汇总或展示，即使功能重心不同，也至少属于 medium 的 same_repository 重复；只有入口、用户流程和验收结果都明显改变时才可放行。"
        "跨仓库必须更保守：触发条件、故障机制和期望结果三者实质相同，或者开发动作、输入校验、失败结果和验收结构至少三项能一一对应且只是替换业务名词时，才可给 duplicate=true 且 confidence=high。"
...
def generate_and_start_iteration(
    ...
    with ITERATION_GENERATION_LOCK:
        if baseline_run_id in ITERATION_GENERATIONS:
            raise WorkflowError("该项目或同一仓库的迭代需求正在生成，请勿重复提交")
    baseline_row = run_row(baseline_run_id)
    baseline_values = dict(baseline_row)
    repository_token = repository_generation_token(
        baseline_values.get("repo_url"), baseline_values.get("repo_name")
    )
    generation_tokens = {baseline_run_id}
    if repository_token:
        generation_tokens.add(repository_token)
    with ITERATION_GENERATION_LOCK:
        if ITERATION_GENERATIONS.intersection(generation_tokens):
            raise WorkflowError("该项目或同一仓库的迭代需求正在生成，请勿重复提交")
        ITERATION_GENERATIONS.update(generation_tokens)
    try:
        try:
            baseline_context = iteration_project_context(baseline_row)
        ...
        with ITERATION_GENERATION_LOCK:
            ITERATION_GENERATIONS.difference_update(generation_tokens)
...
def queue_automatic_iteration_locked(
    ...
    source_repository_token = run_repository_generation_token(baseline_run_id)
    if source_repository_token:
        for other in iteration_job_values():
            if other.get("status") != "generating":
                continue
            other_baseline = str(
                other.get("baseline_run_id")
                or other.get("source_run_id")
                or ""
            )
            if (
                other_baseline != baseline_run_id
                and run_repository_generation_token(other_baseline)
                == source_repository_token
            ):
                raise WorkflowError("同一 GitHub 仓库正在生成另一道题，请等待候选落库")
    job = {
        "status": "generating",
        "source_run_id": source_run_id,
        "baseline_run_id": baseline_run_id,
...
def auto_refill_iteration_candidate(
    *, difficulty_recovery_only: bool = False
    ...
    with db_connection() as database:
        ...
        phase_placeholders = ",".join("?" for _ in SCHEDULED_RUN_PHASES)
        active_repository_rows = database.execute(
            f"""SELECT repo_url, repo_name FROM runs
                  WHERE deleted_at IS NULL
                    AND phase IN ({phase_placeholders})
                    AND COALESCE(repo_url, '') != ''""",
            tuple(SCHEDULED_RUN_PHASES),
        ).fetchall()
    now_epoch = int(time.time())
    ...
    busy_repository_tokens = {
        ...
        for row in active_repository_rows
    ...
    for job in jobs:
        ...
        if token:
            busy_repository_tokens.add(token)
    with ITERATION_GENERATION_LOCK:
        busy_repository_tokens.update(
            token for token in ITERATION_GENERATIONS if token.startswith("repo:")
        )
...
def completed_turn_evaluation_repair_prerequisite_error(
    row: Dict[str, Any],
    *,
    use_cached_trajectory_digest: bool = False,
) -> str:
    ...
    try:
        actual_digest = (
            _cached_trajectory_sha256(trajectory_path)
            if use_cached_trajectory_digest
            else hashlib.sha256(trajectory_path.read_bytes()).hexdigest()
        )
    except OSError as exc:
...
def completed_turn_evaluation_repair_state(
    ...
    *,
    ...
    prerequisite_error = completed_turn_evaluation_repair_prerequisite_error(
        row,
        use_cached_trajectory_digest=use_cached_trajectory_digest,
    )
...
def _completed_turn_record_revision(row: Dict[str, Any]) -> str:
    """Fingerprint list dependencies without reopening the trajectory."""
    trajectory_value = str(
        row.get("turn_trajectory_path") or row.get("run_trajectory_path") or ""
    ).strip()
    trajectory_path = Path(trajectory_value).expanduser() if trajectory_value else None
...
def _completed_turn_record(row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if repair_locked:
        # Remote-locked records cannot be rewritten locally, so the list does
        # not need to reopen their immutable trajectories to rediscover that.
        repairable_issues = []
    ...
    export_ready, export_issues = export_readiness(
        ...
        use_cached_trajectory_digest=True,
    )
...
def completed_turns() -> List[Dict[str, Any]]:
    ...
    # Serialize cache misses so concurrent browser refreshes do not each scan
...
"""
    repair_schema = (
        evaluation_split_dimension_schema(
            dimension_key, include_source_marker=False
        )
        if score_stage_v2
        else {
            "type": "object",
    ...
    try:
        return (
            normalize_evaluation_with_targeted_repairs(
                ...
                repair_notifier,
                supplemental_evidence=supplemental_evidence,
                initial_repair_issues=initial_repair_issues,
            ),
            "",
        )
    except WorkflowError as exc:
...
def evaluation_split_dimension_schema(
    dimension_key: str, *, include_source_marker: bool = True
) -> Dict[str, Any]:
    ...
    if not include_source_marker:
        schema["properties"].pop("descriptionUsesIndependentReview")
        schema["required"].remove("descriptionUsesIndependentReview")
    return schema
...
def evaluation_score_cap_schema() -> Dict[str, Any]:
    ...
    return {
        ...
        "properties": {
            ...
            **{
                key: evaluation_split_dimension_schema(
                    key, include_source_marker=False
                )
                for key in EVALUATION_DIMENSION_KEYS
            },
        },
...
{material}"""
    ...
    initial_repair_issues: List[str] = []
    for dimension_key in EVALUATION_DIMENSION_KEYS:
        ...
        if evaluation_public_description_needs_source_repair(item):
            initial_repair_issues.append(
                f"{EVALUATION_DIMENSION_LABELS[dimension_key]}描述引用后续独立复核证据但没有注明来源"
            )
    ...
    if initial_repair_issues:
        evaluation["_initial_repair_issues"] = initial_repair_issues
    return evaluation
...
def run_codex_regrade(
    ...
    *,
    ...
    initial_repair_issues = result.pop("_initial_repair_issues", [])
    normalized, warning = review_evaluation_with_manual_fallback(
        ...
        initial_repair_issues=(
            initial_repair_issues if isinstance(initial_repair_issues, list) else []
        ),
    )
    score_cap_warning = ""
...
def run_codex_review(
    ...
    findings_trajectory_limit = (
        ...
        if findings_reasoning_effort != FIRST_REVIEW_REASONING_EFFORT
...
def review_worker(run_id: str) -> None:
    try:
        ...
        if (
            ...
            add_event(
                run_id,
                "首轮找 Bug 输出未完成，重试改用 24000 字符紧凑轨迹和中等推理强度",
                "warning",
            )
...
def final_review_worker(run_id: str) -> None:
    try:
        ...
        if (
            ...
            add_event(
                run_id,
                f"第 {turn_number} 轮找 Bug 输出未完成，重试改用 24000 字符紧凑轨迹和中等推理强度",
                "warning",
            )
        elif len(trajectory) > REVIEW_FINDINGS_TRAJECTORY_MAX_CHARS:
            add_event(
        ...
        if not STATUS_CACHE or time.time() - STATUS_CACHE_AT > 60:
            ...
            STATUS_CACHE = {
                ...
                "task_difficulty_policy": (
                    "只启动难度余量为明确困难或地狱的候选；最终按每轮真实轨迹和产物独立评定；"
